...
class MultilingualDatasetManager(object):
    ...
    def load_lang_dataset(
        ...
        for k in itertools.count():
            ...
            elif self.split_exists(split_k, f"{src}_src", f"{tgt}_tgt", f"{src}_src", data_path, dataset_impl):
                src = f"{src}_src"
                tgt = f"{tgt}_tgt"
    ...
    def get_split_num_data_shards(self, split):
        ...
        for data_category, paths in data_paths.items():
            ...
            for src, tgt in lang_dirs:
                if self.args.task == "summarization_multi_simple_epoch" and src == tgt:
                    src = f"{src}_src"
                    tgt = f"{tgt}_tgt"
                ...
                if "mono_" in data_category:
                    ...
                    elif f"{src}-None" in shards_dict or f"None-{src}" in shards_dict:  #Parallel_bt
    ...
    def get_split_data_param_list(self, split, epoch, shard_epoch=None):
        ...
        for data_category, paths in data_paths.items():
            ...
            for src, tgt in lang_dirs:
                ...
                if self.args.task == "summarization_multi_simple_epoch" and src == tgt:
                    key = self.get_dataset_key(data_category, f"{src}_src", f"{tgt}_tgt")
                else:
                    key = self.get_dataset_key(data_category, src, tgt)
    ...
    def get_train_sampling_ratios(
        ...
        if getattr(self.args, 'data_param_list_sampling_ratios', None) is None:
            ...
            if self.args.extra_lang_pairs is not None:
                logger.info("Equal Training Weight for All Data Param List: {}".format(["main"] + list(self.args.extra_lang_pairs.keys())))
        else:
    ...
    def load_split_datasets(
        ...
        self.train_parallel_bt.clear()
        logger.info("Clearing cached parallel bt!")
        datasets = [
            (
                param["key"],
                self.load_a_dataset(
