...
class BARTModel(TransformerModel):
    ...
    def upgrade_state_dict_named(self, state_dict, name):
        ...
        #Expanding Word Embedding
        cur_state_dict = self.state_dict()
        keys = [k for k in cur_state_dict.keys() if "embed_tokens" in k or "decoder.output_projection" in k]
        cur_words_num, embed_size = cur_state_dict[keys[0]].size()
        ...
        if cur_words_num > prev_words_num:
            ...
            logger.info(f"Expanding Word Embedding: {cur_words_num} -> {prev_words_num}")
        else:
            embed_matrix = state_dict[keys[0]][:cur_words_num]
