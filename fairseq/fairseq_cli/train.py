...
def main(cfg: FairseqConfig) -> None:
    ...
    if cfg.task._name == "xnli":
        for valid_sub_split in cfg.dataset.valid_subset.split(","):
            for lg in cfg.task.eval_langs.split(','):
                task.load_dataset(valid_sub_split, lg=lg, combine=False)
...
def validate(
    ...
    valid_losses = []
    if cfg.task._name == "xnli":
        subsets = [f"{subset}_{lg}" for subset in subsets for lg in cfg.task.eval_langs.split(',')]
    for subset in subsets:
        ...
        if cfg.task._name == "xnli":
            if subset not in multilingual_valid_losses:
                ...
                multilingual_valid_losses[subset].append(stats[cfg.checkpoint.best_checkpoint_metric])
            stats[f"best_{subset}"] = max(multilingual_valid_losses[subset])
        progress.print(stats, tag=subset, step=trainer.get_num_updates())
        valid_losses.append(stats[cfg.checkpoint.best_checkpoint_metric])
    if cfg.task._name == "xnli":
        max_multilingual_valid_losses = []
        for lg in multilingual_valid_losses.keys():
