...
PATH_PREFIX = '/mnt/output/glue/glue_data/'

def evaluate_mnli(pred_file, matched_file, mismatched_file, skip_gold=0, skip_pred=0, gold_map=None):
    m_golds = []
...
def evaluate(gold_file, pred_file, metrics=['acc'], skip_gold=0, skip_pred=0, gold_map=None, task="cola"):
    golds = []
    preds = []
...
def evaluate_sts(gold_file, pred_file, skip_gold=1, skip_pred=0, gold_map=None):
    golds = []
    preds = []
    with open(gold_file) as gold_fh:
        for _ in range(skip_gold):
    ...
    assert len(golds) == len(preds), f"{len(golds)} {len(preds)}"
    n_exs = len(golds)
    corr = pearsonr(golds, preds)[0]
    print("pearson r: %.3f" % corr)
    corr = spearmanr(golds, preds)[0]
...
def parse_args():
    ...
    parser.add_argument('--output', '-output', type=str,
                        default="/mnt/output/glue/results/CoLA_dev.tsv",
                        help='main languages file')
    parser.add_argument('--tasks', '-tasks', type=str,
                        default="CoLA",
                        help='main languages file')
...
if 'MSRP' in tasks or 'benchmark' in tasks:
    print('MSRP')
    GOLD_FILE = PATH_PREFIX + 'MSRP/dev.tsv'
...
# SST
if 'SST-2' in tasks or 'benchmark' in tasks:
    print('SST')
...
# STS-B
if 'STS-B' in tasks or 'benchmark' in tasks:
    print('STS-B')
    GOLD_FILE = PATH_PREFIX + 'STS-B/dev.tsv'
    PRED_FILE = args.output
    evaluate_sts(GOLD_FILE, PRED_FILE)


# CoLA
if 'CoLA' in tasks or 'benchmark' in tasks:
    print("CoLA")
    GOLD_FILE = PATH_PREFIX + 'CoLA/dev.tsv'
    PRED_FILE = args.output
    evaluate(GOLD_FILE, PRED_FILE, metrics=['matthews'], task=tasks)
