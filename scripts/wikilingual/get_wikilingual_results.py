...
def read_log(model_name):
    ...
    assert len(SIMPLE_LANGS) == len(ROUGE1_results) and len(SIMPLE_LANGS) == len(ROUGE2_results) and len(SIMPLE_LANGS) == len(ROUGEL_results), "{} | {} | {} | {}".format(len(BLEU_results), len(ROUGE1_results), len(ROUGE2_results), len(ROUGEL_results))
...
if __name__ == "__main__":
    ...
    for i, lg in enumerate(SIMPLE_LANGS):
        latex.append(f"{multilingual_transformer_results[0][i]}/{multilingual_transformer_results[1][i]}/{multilingual_transformer_results[2][i]}")
    latex = " & ".join(latex)
    print(latex)
