    ...
    if os.path.isdir(args.input):
        ...
        for file in files:
            ...
            with open(os.path.join(args.input, file), "r", encoding="utf-8") as input:
                ...
                output_info = "{} | Average Length: {} | Exceed Examples {} | min_len {} | max_len {}".format(file,
                                                                                                              int(sum(
                                                                                                                  lens) / len(
                                                                                                                  lens)),
                                                                                                              len(exceed_lengths),
        ...
        with open(args.input, 'r', encoding="utf-8") as input:
            ...
            for line in lines:
                length = len(line.split())
                lens.append(length)
            output_info = "{} | Average Length: {} | Exceed Examples {} | min_len {} | max_len {}".format(file, int(sum(
                lens) / len(lens)), len(exceed_lengths), min_len, max_len)
            output_infos.append(output_info)
            print(output_info)
