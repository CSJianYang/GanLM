...
python train.py $DATA_DIR \
    ...
    --task glue --criterion sentence_prediction --arch ${ARCH} --log-interval 100 --log-format "simple" \
    --save-interval-updates 1000000 --keep-interval-updates 1 --save-interval 1 --keep-best-checkpoints 5 \
    --init-token 0 --separator-token 2 --num-classes 1 --add-prev-output-tokens --regression-target \
    ...
    --warmup-updates ${WARMUP_STEPS} --max-epoch ${MAX_EPOCH} --best-checkpoint-metric loss --maximize-best-checkpoint-metric \
    --ddp-backend=no_c10d --fp16 --log-file $OUTPUT_PATH/train.log 2>&1 | tee -a $OUTPUT_PATH/all.log

#"en,bg,de,el,en,es,fr,hi,ru,sw,th,tr,ur,vi,zh"
