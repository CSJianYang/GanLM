GLUE_DATA_FOLDER=/home/v-jiaya/unilm-moe/data/glue/glue_data/
SPM_MODEL=/mnt/msranlp/shumma/data/160g/sentencepiece.bpe.model
DICT=/mnt/msranlp/shumma/data/160g/dict.txt
PYTHON=/home/v-jiaya/miniconda3/envs/amlt8/bin/python
TASKS=$1 # QQP
DATA_BIN=/home/v-jiaya/unilm-moe/data/glue/data-bin/
if [ "$TASKS" = "ALL" ]
then
  TASKS="QQP MNLI QNLI MRPC RTE STS-B SST-2 CoLA WNLI AX"
fi
...
do
  ...
  do
    ...
    do
      ...
      cat $TASK_DATA_FOLDER/processed/$SPLIT.raw.$LANG | spm_encode --model=$SPM_MODEL --output_format=piece > $TASK_DATA_FOLDER/processed/$SPLIT.$LANG
    done
  done
  ...
  do
    ...
    $PYTHON /home/v-jiaya/unilm-moe/unilm-moe/fairseq/fairseq_cli/preprocess.py \
      ...
      --destdir "${DATA_BIN}/$TASK-bin/$LANG" \
      --workers 60 \
      --srcdict $DICT;
  done
  if [[ "$TASK" !=  "STS-B" ]]
  then
    $PYTHON /home/v-jiaya/unilm-moe/unilm-moe/fairseq/fairseq_cli/preprocess.py \
      ...
      --destdir "${DATA_BIN}/$TASK-bin/label" \
      --workers 60;
  else
    # For STS-B output range is converted to be between: [0.0, 1.0]
    mkdir -p "$TASK-bin/label"
    awk '{print $1 / 5.0 }' "$TASK_DATA_FOLDER/processed/train.label" > "$${DATA_BIN}/TASK-bin/label/train.label"
    awk '{print $1 / 5.0 }' "$TASK_DATA_FOLDER/processed/dev.label" > "$${DATA_BIN}/TASK-bin/label/valid.label"
  fi
done
