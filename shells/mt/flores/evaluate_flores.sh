...
if [ ! $BLEU_DIR ]; then
    BLEU_DIR=/mnt/output/flores/BLEU/
fi

INPUT=/mnt/output/flores/test-sets/devtest-code_spm/valid.${src}
REFERENCE=/mnt/output/flores/test-sets/devtest-code/valid.${tgt}
...
cat $INPUT | python interactive.py $TEXT \
    ...
    --no-repeat-ngram-size 3 --truncate-source --max-source-positions 512 --max-len-b 128 \
...
cat $OUTPUT_PATH/$OUTPUT_FILE | sacrebleu -tok spm $REFERENCE | tee -a $BLEU_DIR/BLEU.${src}-${tgt}