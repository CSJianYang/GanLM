...
INPUT_FILE=/mnt/output/wmt14-en-de/spm_data/${SPLIT}.${src}
DATA=/mnt/output/wmt14-en-de/data-bin/
REFERENCE=/mnt/output/wmt14-en-de/raw_data/${SPLIT}.${tgt}
...
cat $OUTPUT_PATH/$OUTPUT_FILE | sacrebleu -l en-de $REFERENCE

mkdir -p $BLEU_DIR
cat $OUTPUT_PATH/$OUTPUT_FILE | perl $TOKENIZER -threads 20 -a -l ${tgt} | sed  's/-/ - /g' > $OUTPUT_PATH/$OUTPUT_FILE.tok.post
cat $REFERENCE | perl $TOKENIZER -threads 20 -a -l ${tgt} | sed  's/-/ - /g' > $REFERENCE.tok.post
