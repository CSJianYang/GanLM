...
MODEL=/mnt/output/PretrainedModels/multilingual/electra/lr${LR}-bsz$((${MAX_SENTENCES}*${UPDATE_FREQ}*${NODES}*${GPUS}))-ws${WARMUP_STEPS}-wd${WEIGHT_DECAY}/
mkdir -p $MODEL
python -m torch.distributed.launch --nproc_per_node=$GPUS --nnodes=$NODES --node_rank=$OMPI_COMM_WORLD_RANK --master_addr=$MASTER_ADDR --master_port=$MASTER_PORT train.py $TEXT \
    --save-dir $MODEL --tensorboard-logdir $MODEL/logs --arch electra_base --criterion electra \
    ...
    --ddp-backend=no_c10d --rel-pos-buckets 32 --max-rel-pos 128 --rescale-init --mlm-only 2>&1 | tee -a $MODEL/train.log
