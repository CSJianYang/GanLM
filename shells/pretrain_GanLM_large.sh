...
ROOT=/mnt/output/PretrainedModels/electra-encoder-decoder-v6/
bsz=$((${MAX_SENTENCES}*${UPDATE_FREQ}*${NODES}*${GPUS}))
MODEL=$ROOT/poly/large/lr${LR}-bsz${bsz}-ws${WARMUP_STEPS}-wd${WEIGHT_DECAY}-dw${DISCRIMINATOR_WEIGHT}_${DISCRIMINATOR_START_WARMUP_STEPS}_${DISCRIMINATOR_WARMUP_STEPS}-iw${INCONSISTENCY_WEIGHT}_${INCONSISTENCY_START_WARMUP_STEPS}_${INCONSISTENCY_WARMUP_STEPS}-g${GENERATOR_DECODER_LAYERS}d${DISCRIMINATOR_DECODER_LAYERS}${APPEND_CMDS}/
...
python -m torch.distributed.launch --nproc_per_node=$GPUS --nnodes=$NODES --node_rank=$OMPI_COMM_WORLD_RANK --master_addr=$MASTER_ADDR --master_port=$MASTER_PORT train.py $TEXT \
    --save-dir $MODEL --tensorboard-logdir $MODEL/logs --arch electra_encoder_decoder_v6_large --criterion electra_encoder_decoder_v6 \
    ...
    --log-file $MODEL/train.log ${APPEND_CMDS} 2>&1 | tee -a $MODEL/all.log
