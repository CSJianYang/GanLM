...
MODEL=/mnt/output/PretrainedModels/electra-encoder-decoder/lr${LR}-bsz$((${MAX_SENTENCES}*${UPDATE_FREQ}*${NODES}*${GPUS}))-ws${WARMUP_STEPS}-wd${WEIGHT_DECAY}-dw${DISCRIMINATOR_WEIGHT}/
mkdir -p $MODEL
python -m torch.distributed.launch --nproc_per_node=$GPUS --nnodes=$NODES --node_rank=$OMPI_COMM_WORLD_RANK --master_addr=$MASTER_ADDR --master_port=$MASTER_PORT train.py $TEXT \
    --save-dir $MODEL --tensorboard-logdir $MODEL/logs --arch electra_encoder_decoder_base --criterion electra_encoder_decoder --discriminator-weight $DISCRIMINATOR_WEIGHT \
