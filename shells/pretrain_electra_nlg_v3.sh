...
ROOT=/mnt/output/PretrainedModels/electra-encoder-decoder-v3/
bsz=$((${MAX_SENTENCES}*${UPDATE_FREQ}*${NODES}*${GPUS}))
MODEL=$ROOT/lr${LR}-bsz${bsz}-ws${WARMUP_STEPS}_${DISCRIMINATOR_START_WARMUP_STEPS}_${INCONSISTENCY_START_WARMUP_STEPS}-wd${WEIGHT_DECAY}-dw${DISCRIMINATOR_WEIGHT}-dws${DISCRIMINATOR_WARMUP_STEPS}-iw${INCONSISTENCY_WEIGHT}-iws${INCONSISTENCY_WARMUP_STEPS}-g${GENERATOR_DECODER_LAYERS}d${DISCRIMINATOR_DECODER_LAYERS}${APPEND_CMDS}--wo-rescale-init/
echo $MODEL
mkdir -p $MODEL
python -m torch.distributed.launch --nproc_per_node=$GPUS --nnodes=$NODES --node_rank=$OMPI_COMM_WORLD_RANK --master_addr=$MASTER_ADDR --master_port=$MASTER_PORT train.py $TEXT \
    --save-dir $MODEL --tensorboard-logdir $MODEL/logs --arch electra_encoder_decoder_v3_base --criterion electra_encoder_decoder_v3 \
...
# 2>&1 | tee -a $MODEL/train.log --fp16-scale-window 256 --rescale-init --scale-fc 
