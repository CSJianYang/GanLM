...
TINY_GENERATOR_DECODER_LAYERS=${13}
GENERATOR_DECODER_LAYERS=${14}
DISCRIMINATOR_DECODER_LAYERS=${15}
DISCRIMINATOR_START_WARMUP_STEPS=${16}
INCONSISTENCY_START_WARMUP_STEPS=${17}
DISCRIMINATOR_SAMPLING_TEMPERATURE=${18}
SEED=${19}
APPEND_CMDS=${20}
GPUS=8
MAX_EPOCH=1000
...
ROOT=/mnt/output/PretrainedModels/electra-encoder-decoder-v8/
bsz=$((${MAX_SENTENCES}*${UPDATE_FREQ}*${NODES}*${GPUS}))
MODEL=$ROOT/poly/lr${LR}-bsz${bsz}-ws${WARMUP_STEPS}-wd${WEIGHT_DECAY}-dw${DISCRIMINATOR_WEIGHT}_${DISCRIMINATOR_START_WARMUP_STEPS}_${DISCRIMINATOR_WARMUP_STEPS}-iw${INCONSISTENCY_WEIGHT}_${INCONSISTENCY_START_WARMUP_STEPS}_${INCONSISTENCY_WARMUP_STEPS}-g${TINY_GENERATOR_DECODER_LAYERS}g${GENERATOR_DECODER_LAYERS}d${DISCRIMINATOR_DECODER_LAYERS}${APPEND_CMDS}/
...
python -m torch.distributed.launch --nproc_per_node=$GPUS --nnodes=$NODES --node_rank=$OMPI_COMM_WORLD_RANK --master_addr=$MASTER_ADDR --master_port=$MASTER_PORT train.py $TEXT \
    --save-dir $MODEL --tensorboard-logdir $MODEL/logs --arch electra_encoder_decoder_v8_base --criterion electra_encoder_decoder_v8 \
    ...
    --generator-decoder-layers ${GENERATOR_DECODER_LAYERS} --tiny-generator-decoder-layers ${TINY_GENERATOR_DECODER_LAYERS} \
    --discriminator-decoder-layers ${DISCRIMINATOR_DECODER_LAYERS} --discriminator-sampling-temperature ${DISCRIMINATOR_SAMPLING_TEMPERATURE} \
