...
@register_criterion("electra_encoder_decoder", dataclass=ElectraConfig)
class ElectraEncoderDecoderLoss(FairseqCriterion):
    """
    ...
    def mask_lm_loss(self, model, sample, reduce, update_num):
        masked_tokens = sample["src_tokens"].eq(self.mask_idx)
        sample_size = masked_tokens.int().sum().detach().tolist()
        ...
        input_tokens, generator_logits, discriminator_logits = encoder_output["electra"][:3]
    ...
    def forward(self, model, sample, reduce=True, epoch=-1, update_num=-1):
        ...
        if "mlm" in sample and not self.cfg.seq2seq_only:
            mlm_loss, mlm_sample_size, mlm_logging_output = self.mask_lm_loss(
                model, sample["mlm"], reduce=reduce, update_num=update_num
            )
            loss += mlm_loss
        ...
        if "seq2seq" in sample and not self.cfg.mlm_only:
            ...
            if "mlm" in sample:
                loss += seq2seq_loss * sample_size / seq2seq_sample_size
                ...
                logging_outputs["sample_size"] += seq2seq_logging_output["sample_size"]
