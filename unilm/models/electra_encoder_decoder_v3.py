...
@dataclass
class ElectraEncoderDecoderConfigv3(FairseqDataclass):
...
class ElectraEncoderv3(TransformerEncoderBase):
    def __init__(
            self,
            args,
            dictionary,
...
class ElectraDecoderv3(UniLMDecoder):
    """
    Transformer decoder consisting of *cfg.decoder.layers* layers. Each layer
    is a :class:`TransformerDecoderLayer`.
    ...
    def forward(
        ...
        with torch.no_grad():
            sampled_probs = torch.softmax(generator_logits.view(-1, generator_logits.size(-1))/self.args.sampling_temperature, -1, dtype=torch.float32)
            sampled_tokens = torch.multinomial(sampled_probs, 1).view(-1)
...
@register_model("electra_encoder_decoder_v3")
class ElectraEncoderDecoderv3(TransformerModel):
    """
    ...
    def add_args(cls, parser):
        ...
        gen_parser_from_dataclass(
            parser, ElectraEncoderDecoderConfigv3(), delete_default=False, with_prefix=""
        )
    ...
    def build_model(cls, args, task):
        ...
        base_electra_encoder_decoder_architecture_v3(args)
    ...
    def build_encoder(cls, cfg, src_dict, embed_tokens):
        ...
        return ElectraEncoderv3(wrapper_cfg, src_dict, embed_tokens)


    @classmethod
    def build_decoder(cls, cfg, tgt_dict, embed_tokens):
        return ElectraDecoderv3(
            cfg,
            tgt_dict,
            embed_tokens,
    ...
    def upgrade_state_dict_named(self, state_dict, name):
        ...
        if "discriminator.embed_tokens.weight" in state_dict.keys(): # Electra
            ...
            if self.args.initialization_strategy == "discriminator":
                ...
                for k, v in cur_state_dict.items():
                    state_dict[k] = v
                logger.info("Directly Loading Checkpoint without Any Change | Deleting Decoder Generator")
                ...
                for k, v in cur_state_dict.items():
                    state_dict[k] = v
                logger.info("Directly Loading Checkpoint without Any Change | Deleting Decoder Discriminator")
        else:
...
@register_model_architecture("electra_encoder_decoder_v3", "electra_encoder_decoder_v3")
def electra_encoder_decoder_architecture_v3(args):
...
@register_model_architecture("electra_encoder_decoder_v3", "electra_encoder_decoder_v3_base")
def base_electra_encoder_decoder_architecture_v3(args):
    electra_encoder_decoder_architecture_v3(args)


@register_model_architecture("electra_encoder_decoder_v3", "electra_encoder_decoder_v3_large")
