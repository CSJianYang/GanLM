...
@dataclass
class ElectraEncoderDecoderConfigv8(FairseqDataclass):
    ...
    tiny_generator_decoder_layers: int = field(
        default=4,
        metadata={"help": "share parameters"}
    )
...
class ElectraEncoderv8(TransformerEncoderBase):
    def __init__(
            self,
            args,
            dictionary,
...
class ElectraDecoderv8(UniLMDecoder):
    """
    Transformer decoder consisting of *cfg.decoder.layers* layers. Each layer
    is a :class:`TransformerDecoderLayer`.
    ...
    def __init__(
        ...
        if args.task == 'pretraining':
            tiny_generator_args = copy.copy(args)
            if getattr(args, "tiny_generator_decoder_layers", -1) == -1:
                tiny_generator_args.tiny_generator_decoder_layers = args.decoder_layers
            else:
                tiny_generator_args.decoder_layers = args.tiny_generator_decoder_layers
            self.tiny_generator = ElectraDecoderBase(
                tiny_generator_args,
                dictionary,
                embed_tokens,
    ...
    def forward(
        ...
        if self.tiny_generator is not None:
            generator_out = self.tiny_generator(
                prev_output_tokens,
                encoder_out=encoder_out,
        ...
        with torch.no_grad():
            sampled_probs = torch.softmax(generator_logits.view(-1, generator_logits.size(-1))/self.args.discriminator_sampling_temperature, -1, dtype=torch.float32)
            sampled_tokens = torch.multinomial(sampled_probs, 1).view(-1)
            sampled_tokens = sampled_tokens[masked_tokens.view(-1)]
            input_tokens = targets.clone()
            input_tokens[masked_tokens] = sampled_tokens
            #
            sampled_probs = sampled_probs.scatter(-1, targets.view(sampled_probs.size(0), 1), 0)
...
@register_model("electra_encoder_decoder_v8")
class ElectraEncoderDecoderv8(TransformerModel):
    """
    ...
    def add_args(cls, parser):
        ...
        gen_parser_from_dataclass(
            parser, ElectraEncoderDecoderConfigv8(), delete_default=False, with_prefix=""
        )
    ...
    def build_model(cls, args, task):
        ...
        base_electra_encoder_decoder_architecture_v8(args)
    ...
    def build_encoder(cls, cfg, src_dict, embed_tokens):
        ...
        return ElectraEncoderv8(wrapper_cfg, src_dict, embed_tokens)


    @classmethod
    def build_decoder(cls, cfg, tgt_dict, embed_tokens):
        return ElectraDecoderv8(
            cfg,
            tgt_dict,
            embed_tokens,
    ...
    def upgrade_state_dict_named(self, state_dict, name):
        def upgrade_embed_tokens(embed_k, cur_state_dict, state_dict, prefix=""):
            keys = [k for k in cur_state_dict.keys() if "embed_tokens" in k or "decoder.generator.output_projection" in k]
            ...
            if cur_words_num > prev_words_num:
                ...
                embed_matrix = state_dict[embed_k]
            for k in keys:
        ...
        if "discriminator.embed_tokens.weight" in state_dict.keys(): # Electra
            ...
            upgrade_embed_tokens("encoder.discriminator.embed_tokens.weight", cur_state_dict, state_dict, prefix="generator")
            if self.args.initialization_strategy == "discriminator":
                ...
                if "decoder.discriminator.embed_positions.weight" in state_dict.keys() and self.args.decoder_learned_pos:
                    upgrade_position_embed("decoder.discriminator.embed_positions.weight", state_dict, state_dict, prefix="decoder", new_prefix="decoder", max_positions=self.args.max_target_positions + 2)
                for k in state_dict.keys():
                    if not k.startswith("decoder.generator.") and not k.startswith("decoder.discriminator.output_projection"):
                        assert k in cur_state_dict, k
                        cur_state_dict[k] = state_dict[k]
                cur_state_dict['decoder.discriminator.output_projection.weight'] = state_dict["decoder.discriminator.embed_tokens.weight"] #share all embeddings
                state_dict.clear()
                for k, v in cur_state_dict.items():
                    state_dict[k] = v
                ...
                if "decoder.generator.embed_positions.weight" in state_dict.keys() and self.args.decoder_learned_pos:
                    upgrade_position_embed("decoder.generator.embed_positions.weight", state_dict, state_dict, prefix="decoder", new_prefix="decoder", max_positions=self.args.max_target_positions + 2)
                for k in state_dict.keys():
...
@register_model_architecture("electra_encoder_decoder_v8", "electra_encoder_decoder_v8")
def electra_encoder_decoder_architecture_v8(args):
...
@register_model_architecture("electra_encoder_decoder_v8", "electra_encoder_decoder_v8_base")
def base_electra_encoder_decoder_architecture_v8(args):
    electra_encoder_decoder_architecture_v8(args)


@register_model_architecture("electra_encoder_decoder_v8", "electra_encoder_decoder_v8_large")
