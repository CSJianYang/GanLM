...
@dataclass
class XNLIConfig(FairseqDataclass):
    ...
    train_langs: str = field(
        default="en",
        metadata={"help": "max tokens per example"},
    )
    eval_langs: str = field(
        default="en",
        metadata={"help": "max tokens per example"},
    )
...
@register_task("xnli", dataclass=XNLIConfig)
class XNLISentencePredictionTask(FairseqTask):
    """
    ...
    def setup_task(cls, cfg, **kwargs):
        ...
        data_dict = cls.load_dictionary(
            os.path.join(cfg.data, cfg.train_langs.split(',')[0], "input0", "dict.txt"), add_mask_token=True
        )
        ...
        if not cfg.regression_target:
            label_dict = cls.load_dictionary(
                os.path.join(cfg.data, cfg.train_langs.split(',')[0], "label", "dict.txt"), add_mask_token=False
            )
    ...
    def load_dataset(self, split, lg=None, combine=False, **kwargs):
        """Load a given dataset split (e.g., train, valid, test)."""

        def get_path(key, split):
            return os.path.join(self.cfg.data, key, split)

        def make_dataset(key, dictionary, lg="en"):
            try:
                if split == "train":
                    datasets = []
                    for lg in self.cfg.train_langs.split(','):
                        split_path = get_path(f"{lg}/{key}", split)
        ...
        if not self.cfg.regression_target:
            label_dataset = make_dataset("label", self.label_dictionary, lg=lg)
        ...
        if lg is not None:
            self.datasets[f"{split}_{lg}"] = dataset
            return self.datasets[f"{split}_{lg}"]
        else:
